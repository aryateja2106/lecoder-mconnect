...
import {
  ...
  checkCommand,
  filterCommand,
  getGuardrailDescription,
  isGuardrailsDisabled,
} from './index.js';
...
describe('loadGuardrails', () => {
  ...
  test('returns strict config for "strict" level', () => {
    ...
    expect(config.approvalPatterns.length).toBeGreaterThan(
      loadGuardrails('default').approvalPatterns.length
    );
  });
  ...
  test('returns permissive config for "permissive" level', () => {
    ...
    expect(config.approvalPatterns.length).toBeLessThan(
      loadGuardrails('default').approvalPatterns.length
    );
  });
