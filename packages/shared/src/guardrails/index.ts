 ...
 */
const BLOCKED_PATTERNS_BASE: RegExp[] = [
 ...
 */
const APPROVAL_PATTERNS: Record<GuardrailLevel, RegExp[]> = {
  none: [],
  permissive: [
 ...
 */
export function loadGuardrails(level: GuardrailLevel | string): GuardrailConfig {
 ...
 */
function normalizeLevel(level: string): GuardrailLevel {
  const normalized = level.toLowerCase().trim();
  if (['none', 'permissive', 'default', 'strict'].includes(normalized)) {
    return normalized as GuardrailLevel;
  }
  return 'default';
}
...
): { allowed: boolean; command: string | null; check: CommandCheck } {
 ...
 */
export function getBlockedPatterns(level: GuardrailLevel): RegExp[] {
 ...
 */
export function getApprovalPatterns(level: GuardrailLevel): RegExp[] {
 ...
 */
export function isGuardrailsDisabled(level: GuardrailLevel): boolean {
  return level === 'none';
}
 ...
 */
export function getGuardrailDescription(level: GuardrailLevel): string {
  switch (level) {
    case 'none':
